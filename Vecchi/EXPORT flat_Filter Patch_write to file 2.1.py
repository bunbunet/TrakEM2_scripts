...
Directory="G:\\Progetto QA\\RR\\RR2\\Export\\QuickNII\\"
#Specify additional tag in file name
tag="_quickNII"

# LAYER RANGE
index_first_section = 1
index_last_section = 137
...
#makeFlatImage starts from the original images
  ip = loader.getFlatImage(layer, bounds, scale, -1,ImagePlus.GRAY8, Displayable, False)
  if QuikNII:
  	FileSaver(ip).saveAsPng(targetDir + "_s"+str(format(i + index_first_section, pad)) + ".png")
  else:
  	FileSaver(imp).saveAsTiff(targetDir + "_z"+str(i + index_first_section) + ".tif")
